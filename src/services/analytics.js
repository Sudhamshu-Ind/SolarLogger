import { safeFormatDate, safeParseDate } from './proRataEngine';
...
export function calculateSummaryMetrics(dailySeries = [], rawEntries = [], blocks = [], settings = {}) {
  ...
  let mtdUnits = 0;
  const now = new Date();
  const currentMonthStr = safeFormatDate(now, 'yyyy-MM');
  ...
  dailySeries.forEach((day) => {
    lifetimeUnits += (day.totalDailyUnits || 0);

    if (day.date && day.date.startsWith(currentMonthStr)) {
      mtdUnits += (day.totalDailyUnits || 0);
    }

    (blocks || []).forEach((b) => {
      const bData = day.blockData ? day.blockData[b.id] : null;
  ...
  const todayUnits = latestDay ? (latestDay.totalDailyUnits || 0) : 0;
...
export function aggregateMonthlyData(dailySeries = [], blocks = []) {
  ...
  (dailySeries || []).forEach((day) => {
    ...
    if (!monthMap.has(monthKey)) {
      const monthLabel = safeFormatDate(`${monthKey}-01`, 'MMM yyyy', monthKey);
      const record = {
        monthKey,
        monthLabel,
        totalUnits: 0,
    ...
    rec.totalUnits += (day.totalDailyUnits || 0);
...
export function formatDailyChartData(dailySeries = [], blocks = []) {
  return (dailySeries || []).map((day) => {
    const formattedDate = safeFormatDate(day.date, 'dd MMM', day.date || '');
    const item = {
      date: day.date,
      formattedDate,
      total: day.totalDailyUnits || 0,
      isEstimated: day.isEstimated || false,
      hasEstimatedBlock: day.hasEstimatedBlock || false,
    };

    (blocks || []).forEach((b) => {
      const bData = day.blockData ? day.blockData[b.id] : null;
      item[b.id] = bData ? (bData.dailyUnits || 0) : 0;
      item[`${b.id}_cumulative`] = bData ? (bData.cumulativeUnits || 0) : 0;
      item[`${b.id}_isEstimated`] = bData ? Boolean(bData.isEstimated) : false;
    });
